...
class Events
{
    ...
    public static function onWorkerStart($worker)
    {
        ...
        if (empty(self::$global)) {
            self::$global
                = new \GlobalData\Client(self::$global_cfg['globalData']);
            // 客服列表
    ...
    public static function onMessage($client_id, $message)
    {
        ...
        switch ($message['type']) {
            ...
            case 'init':
                ...
                // 如果该客服未在内存中记录则记录
                if (!isset($kfList[$message['group']])
                    || !array_key_exists($message['uid'],
                        $kfList[$message['group']])
                ) {
                    ...
                    } while (!self::$global->cas('kfList', $kfList,
                        $newKfList));
                    ...
                    if (isset($kfList[$message['group']][$message['uid']])) {
                        ...
                        do {
                            $newKfList = $kfList;
                            $newKfList[$message['group']][$message['uid']]['client_id']
                                = $client_id;
                        } while (!self::$global->cas('kfList', $kfList,
                            $newKfList));
                        unset($newKfList, $kfList);
            ...
            case 'userInit';
                ...
                if (!array_key_exists($message['uid'], $userList)) {
                    ...
                    } while (!self::$global->cas('userList', $userList,
                        $NewUserList));
                    ...
                    } while (!self::$global->cas('uidSimpleList', $old,
                        $newList));
                    unset($old, $newList);
            ...
            case 'chatMessage':
                ...
                if (!empty($client)) {
                    ...
                    Gateway::sendToClient($client['0'],
                        json_encode($chat_message));
                    unset($chat_message);
                    ...
                    self::$db->insert('ws_chat_log')->cols($serviceLog)
                        ->query();
                    unset($serviceLog);
                }
                break;
            ...
            case 'changeGroup':
                ...
                $userGroup
                    = $simpleList[$message['uid']]['1'];  // 会员原来的分组也是客服的分组
                ...
                // 记录该客服与该会员的服务结束
                self::$db->query("update `ws_service_log` set `end_time` = "
                    . time() . " where `client_id`= '" . $userClient . "'");
                ...
                if (!isset($kfList[$userGroup])) {
                    ...
                    foreach (self::$global->userList as $vo) {
                        ...
                        Gateway::sendToClient($userClient,
                            json_encode($waitMessage));
                        unset($waitMessage);
                    }
                    return;
                }
            ...
            case 'closeUser':
                ...
                if (isset($userInfo[$message['uid']])) {
                    ...
                    Gateway::sendToClient($userInfo[$message['uid']]['0'],
                        json_encode($waitMessage));
                    unset($waitMessage);
                }
                unset($userInfo);
            ...
            case 'ping':
                if (isset($message['data']['l_user_id'])) {
                    $user
                        = self::$db->query("SELECT `status` FROM `ws_users` WHERE `id`={$message['data']['l_user_id']} LIMIT 1");
                    if ($user[0]['status'] == 2) {
                        $waitMessage = [
                            'message_type' => 'close',
                        ];
        ...
        foreach ($kfList as $k => $v) {
            foreach ($v as $key => $vo) {
                if (in_array($client_id, $vo['user_info'])) {
                    ...
                    self::$db->query("update `ws_service_log` set `end_time` = "
                        . time() . " where `client_id`= '" . $client_id . "'");
                    ...
                    while (!self::$global->cas('uidSimpleList', $oldSimple,
                        $simpleList)) {
                    };
                    ...
                    if (!empty($outUser)) {
                        ...
                        Gateway::sendToClient($vo['client_id'],
                            json_encode($del_message));
                        unset($del_message);
    ...
    private static function userOfflineTask($group)
    {
        ...
        $res = self::assignmentTask(self::$global->kfList,
            self::$global->userList, $group, $maxNumber);
        unset($maxNumber);

        if (1 == $res['code']) {

            while (!self::$global->cas('kfList', self::$global->kfList,
                $res['data']['4'])) {
            }; // 更新客服数据
            while (!self::$global->cas('userList', self::$global->userList,
                $res['data']['5'])) {
            }; // 更新会员数据
            // 通知会员发送信息绑定客服的id
            $noticeUser = [
            ...
            Gateway::sendToClient($res['data']['3']['client_id'],
                json_encode($noticeUser));
            unset($noticeUser);
    ...
    private static function userOnlineTask($client_id, $group)
    {
        ...
        if (1 == $res['code']) {
            ...
            // 检测是否开启自动应答
            $sayHello
                = self::$db->query('select `word`,`status` from `ws_reply` where `id` = 1');
...
//                'end_time' => 0
//            ];
//            var_dump($init_service_log);
//            self::$db->delete('ws_service_log')->where($init_service_log)->where('user_id', $res['data']['3']['id'])
//                ->query();
            self::$db->query("delete from `ws_service_log` where `user_id` = '"
                . $res['data']['3']['id'] . "' and `kf_id`= " . intval(ltrim($res['data']['0'], 'KF')) . " and `group_id`= {$group} and end_time = 0");
//            unset($init_service_log);
    ...
    private static function assignmentTask($kfList, $userList, $group, $total)
    {
        ...
        array_push($kfList[$group][$flag]['user_info'],
            $user['client_id']); // 被分配的用户信息
    ...
    {
        $maxNumber
            = self::$db->query('select `max_service` from `ws_config` where `id` = 1');
        if (!empty($maxNumber)) {
            $maxNumber = 5;
        } else {
